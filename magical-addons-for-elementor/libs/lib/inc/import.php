...
if (did_action('elementor/loaded')) {
	class Magcial_Addon_Import_Library  extends Source_Base
	{
		...
		public function xl_tab_import_data()
		{
			...
			//	$data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get ($remote . 'wp-json/mg/v1/' . $end_point . '/?id=' . $id)), true);
			$data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get($remote . 'wp-json/mg/v1/' . $end_point . '/?id=' .
				$id, ['timeout' => 120])), true);
